...
{
    DscArch *a = mock_arch_identity();
    UINT64 phys = 0;

    int rc = DscArchLogicalToPhysical(a, 0x1000, &phys);
...
void identity_physical_equals_logical(void)
{
    ...
    int rc = DscArchPhysicalToLogical(a, 0x2000, &logical);
    TEST_ASSERT_EQUAL(DSC_OK, rc);
...
{
    DscArch *a = mock_arch_identity();
    TEST_ASSERT_EQUAL_size_t(1, DscArchMinAccessSize(a));
}

void identity_word_size_is_one(void)
{
    DscArch *a = mock_arch_identity();
    TEST_ASSERT_EQUAL_size_t(1, DscArchWordSize(a));
}
...
{
    DscArch *a = mock_arch_word16();
    UINT64 phys = 0;

    int rc = DscArchLogicalToPhysical(a, 0x100, &phys);
    TEST_ASSERT_EQUAL(DSC_OK, rc);
...
void word16_physical_to_logical_multiplies_by_two(void)
{
    ...
    int rc = DscArchPhysicalToLogical(a, 0x80, &logical);
    TEST_ASSERT_EQUAL(DSC_OK, rc);
...
void word16_unaligned_returns_error(void)
{
    ...
    int rc = DscArchLogicalToPhysical(a, 0x101, &phys);
...
void word16_zero_address(void)
{
    ...
    int rc = DscArchLogicalToPhysical(a, 0, &phys);
    TEST_ASSERT_EQUAL(DSC_OK, rc);
...
{
    DscArch *a = mock_arch_word16();
    TEST_ASSERT_EQUAL_size_t(2, DscArchMinAccessSize(a));
}

void word16_word_size_is_two(void)
{
    DscArch *a = mock_arch_word16();
    TEST_ASSERT_EQUAL_size_t(2, DscArchWordSize(a));
}
