...
| 比数能力 | 接口 FPGA 实时比 + 高精度比 | 桩 CPU 按 `g_compareBufCompAddr[]` 描述符软比（详见 § 1.6）|
...
class DebugConsole:
    mem: MemAccessAPI
    functions: Dict[str, Callable[..., Any]]   # 被测系统函数注册表（构造时一次性传入）
    default_dtype: _ScalarType = Datatype.UINT32

    def run(self) -> None: ...                  # 阻塞主循环
    def handle(self, line: str) -> None: ...    # 单行 dispatch（测试 / 脚本驱动用）
```

#### 4.9.3 注册表语义：被测系统函数 ≠ Autotest 工具函数

`functions[name]` 注册的是**被测系统侧函数**的 Python wrapper（演示对应桩 CPU 上 `DUT_DBG_ClientHandshake` / `SVC_COMPARE_PullBatch` / `L6B_SoftdebugRead` 等 C 函数）。

| 被测系统 C 函数 | demo Python wrapper 行为 | 真部署对应 |
|---|---|---|
| `DUT_DBG_ClientHandshake` | `MemAccessAPI` 校验 region magic + RTT magic | 桩 CPU `dut_dbg_client.c::DUT_DBG_ClientHandshake` |
| `SVC_COMPARE_PullBatch` | 拉 `g_compareBufDebugCnt` + 描述符数组 | 桩 CPU `svc_compare.c::SVC_COMPARE_PullBatch` |
| `L6B_SoftdebugRead` | `MemAccessAPI.ReadBytes` 一次性读 n 字节 | 接口 FPGA L6B SDK 调用 |

真部署里这层 wrapper 会被 DEBUG 口 RPC 通道替代；REPL 命令语法 / 显示格式不变。

#### 4.9.4 工程约定

- 不引入任何外部库（纯标准库 `shlex` / `io` / `contextlib`）
- 函数注册表构造时一次性传入；要历史 / 补全可外挂 `readline`
- 调函数时捕获被调函数 stdout 当作"DEBUG 回显"展示，返回值单独打印 `=> {ret!r}`
- 命令解析用前缀 dispatch：`d ` → dump，`!` → call，其他 → 查符号
