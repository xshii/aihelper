...
软调通道的硬件协议（opcode / addr / len / payload）由 L6A 内部封装。未来软调协议升级（带宽优化、批量打包），改 L6A 内部即可，L5A 与 L4 svc **无感**。
...
| 符号 | 类型 | 含义 |
|---|---|---|
| `g_debugCnt` | `uint32_t` | 当前有效比数项数（0 = 无新数据，桩 CPU 继续轮询）|
| `g_compAddr[200]` | `compare_entry_t[200]` | 比数项数组（容量 200，消费整体清零后从头复用，详见 § 1.6.3）|

```c
typedef struct {
    uint16_t tid;       // 张量 / 阶段 ID
    uint16_t cnt;       // 同一 tid 的第几次产出（用于阶段性多轮）
    uint32_t length;    // 数据长度（字节）
    void*    addr;      // 数据起始地址（被测 FPGA 内存空间）
} compare_entry_t;      // 32 位平台 12 字节 / 64 位平台 16 字节
```

> `addr` 指向 DUT 内存空间内的张量缓冲，桩 CPU 拿到后再发一次 SoftDebug 读，把 `[addr, addr+length)` 段拉回单板做软比。`g_compAddr` 中**不内联数据**，仅存描述符。
...
sequenceDiagram
    ...
    loop 每个 poll 周期
        L5B->>SD: read g_debugCnt (4B)
        SD->>DUT: SoftDebug read
        DUT-->>L5B: N
        alt N == 0
            Note over L5B: 继续等下一个周期
        else N > 0
            L5B->>SD: read g_compAddr[0..N-1] (N × 12B)
            SD-->>L5B: 描述符数组
            loop i = 0..N-1
...
- **生产侧（DUT 固件）**：每产出一项，**先填 `g_compAddr[g_debugCnt]` 全部 4 个字段，再以原子写递增 `g_debugCnt`**。顺序颠倒会让桩 CPU 读到未初始化项。
- **消费侧（桩 CPU）**：处理完整批后**整体写 0 清 `g_debugCnt`**（不做逐项消费），同时清空本地 batch；固件视为已消费缓冲区。
...
L5B 对外暴露语义化函数（绝不暴露 `g_debugCnt` / `g_compAddr` 地址），内部用 § 4.8 通用 API 实现：

| 函数 | 职责 | 内部展开 |
|---|---|---|
| `mem_drv_pull_compare_batch()` | 完整执行 § 1.6.2 时序，返回 `list[(entry, data)]` | `ReadVal("g_debugCnt", UINT32)` → 循环 `ReadStruct("g_compAddr", CompareEntry, i)` → `ReadArray` 拉数据 |
| `mem_drv_clear_compare_buf()` | 显式清零（异常恢复路径用）| `WriteVal("g_debugCnt", UINT32, 0)` |

M07（机制 B 路径）拿到 batch 后调用桩 CPU 软比函数，按 02 § 4.1.2 输出备选路径报告。符号 → 地址映射由链接 map 解析（详见 § 4.8.3），**绝不在 svc / L4 层硬编码**。

#### 1.6.5 抽象稳定性

未来若固件改用环形队列 / 双 buffer / DMA 推送替代当前轮询契约，仅 L5B 内部适配，L4 svc 与 M07 **无感**。

### 1.7 机制 A 发包内存组合 — `DATA_BUF` 协议

> **关键性**：DATA_BUF 由 N 个 DDR chunk 组成。每 chunk = 512B 头 + payload；大 payload 自动分片。**关键预埋 = 块头 + 分片填写**（§ 1.7.8）；业务层（VPORT 头 / 业务 header / 业务字段）由调用方用 `struct.pack` 自由拼接，**不强加结构**。
...
| 想了解什么 | 直接看 |
|---|---|
| **DDR 块头字段 / 分片如何切** | § 1.7.8（关键预埋；最重要）|
| **怎么把业务 payload 一行发出去** | § 1.7.4 用例视角 |
| **底层 Block 序列化机制（512 对齐 / 端序 / 位域）** | § 1.7.1 ~ § 1.7.7（基础设施）|

**设计模式**：**Composite Pattern + dataclass + 512 字节对齐**；每块自描述、可链式拼接、序列化声明式。

#### 1.7.1 设计目标（Block 序列化抽象层）

> 本节列出 Block 抽象层设计目标。DDR 层（块头 + 分片填写）目标作为关键预埋单列在 § 1.7.8。

| # | 目标 | 落点 |
|---|---|---|
| 1 | 多张量自由顺序组合 | `Block.__add__` / `Composite` |
| 2 | 新 Block 类型不动现有 | 继承 `Block` ABC |
| 3 | **512 字节对齐**强约束 | `Block.__bytes__` 自动 padding |
| 4 | **大小端可配**（FPGA 侧约定）| `Block.ENDIAN` 类常量（`<` / `>`，struct 约定）|
| 5 | **位域支持**（sub-byte 字段）| `bitstruct` 库声明式 / 手工掩码兜底 |
| 6 | 序列化一行 | `bytes(buf)` |
| 7 | 不可变 / 链式安全 | `@dataclass(frozen=True)` |
...
| 写法 | 用途 |
|---|---|
| `__add__` / `__radd__` 重载 | `a + b` 顺序拼接 + `sum(blocks, 0)` 兼容 |
| `__bytes__` / `__len__` 协议 | `bytes(buf)` 触发序列化、`len(buf)` 给对齐后字节数 |
| `(-n) % N` | 对齐填充长度（零分支算法）|
| `struct.pack(f"{ENDIAN}...")` | 字节对齐字段 + 端序前缀 |
| `bitstruct.pack(LAYOUT)` + `byteswap` | 位域声明式打包 + 字节序反转 |
| `Literal["<", ">"]` | 端序参数类型约束（结合 typing）|
| `@dataclass(frozen=True, slots=True)` | 不可变值类型 + 紧凑内存 |
| `ABC + @abstractmethod` | 协议骨架（强制子类实现）|
| `ClassVar[int]` | 类常量不被 dataclass 当字段 |
...
`Block` 基类协议（`_payload` + `ALIGNMENT` + `ENDIAN`）= 全局合约；新增 Block 类型不动协议。Q-001 定型时只在 L5A 加 `@dataclass` 子类（含位域 LAYOUT / 端序覆盖）即可。`ALIGNMENT` / `ENDIAN` 集中改一处全部生效。

#### 1.7.8 DDR 块头 + 分片填写（关键预埋）
...
| 等结果 | 接口 FPGA 软调轮询 done 消息 | SoftDebug 周期 poll `g_debugCnt`（§ 1.6）|
| 比数 | 接口 FPGA 比数引擎实时 | 不支持硬件比，桩 CPU 按 `g_compAddr[]` 描述符软比 |

> **机制 A 关键**：`start_model`（VPORT START 进入运行态）与 `enable_periodic_data_inject`（接口 FPGA 定时器送数）是**两个独立动作**，不要合并。
>
> **机制 B 关键**：DUT 固件**必须**提供「配置区轮询入口」**和**「`g_debugCnt` / `g_compAddr` 比数契约」（§ 1.6）；二者缺一不可。
...
### 3.1 机制 A vs B 全景对比（FpgaAdapter 内部）

| 维度 | 机制 A | 机制 B |
|---|---|---|
| 物理路径 | 桩 CPU → 接口 FPGA → VPORT → 被测 FPGA | 桩 CPU → SoftDebug → 被测 FPGA |
| 数据预处理 | **需要二次加工** | 字节流直写 |
| 启停信号 | VPORT 消息 | 写配置区 + 被测侧轮询 |
| 完成事件 | 完成消息 / 中断 | 桩 CPU 轮询状态字 |
| 数据注入 | 定时器周期发数（自治）| 一次性写入 |
| 比数能力 | 接口 FPGA 实时比 + 高精度比 | 桩 CPU 按 `g_compAddr[]` 描述符软比（详见 § 1.6）|
| 被测固件依赖 | DUT 框架响应 VPORT 消息 | DUT 框架轮询配置区 + 维护 `g_debugCnt` / `g_compAddr[200]` 比数契约（**硬约束**）|
...
| 阶段 | 路径 | 核心点 |
|---|---|---|
| 主机预处理 | PC 内存 | 格式转换、分片、对齐、CRC |
| 下行 PC → 桩 CPU | DEBUG 口 | 流式传，避免单消息过大 |
| 桩 CPU 缓存 | RTOS 内存 / 外接内存 | 用 `buf_id` 索引（M12），可复用 |
| 下行 桩 CPU → 片内 | 机制 A 或 B | A 路径含 **二次加工** |

**二次加工**（机制 A 关键）：通用张量数据 → 接口 FPGA 消息引擎可识别的载荷格式，按 § 1.7 `Block` 组合协议落地（512 对齐 + dataclass + Composite Pattern）。封装在 `msg_drv` 内。具体 Block 类型集见 [Q-001]。
...
> Autotest 不直接拿地址读写 —— 全部通过**符号名 + 类型描述符**调通用 API，由 L5 在底层翻译为 SoftDebug（机制 B）或接口 FPGA 软调（机制 A）。`g_debugCnt` / `g_compAddr` 是这套 API 的典型消费者。
...
ReadBytes(addr: int, n: int) -> bytes      # 绝对地址裸读（地址来自数据时用，如 g_compAddr[i].addr）
...
| 调用方 | 用途 |
|---|---|
| M07（机制 B 比数）| 实现 § 1.6 协议（`g_debugCnt` / `g_compAddr[]`）|
...
| Q-009 | 被测固件机制 B 协议：配置区轮询入口 + `g_debugCnt` / `g_compAddr[200]` 链接地址 | M10-B | Open |
