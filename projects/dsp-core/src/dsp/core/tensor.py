...
if TYPE_CHECKING:
    from .enums import TensorSource
...
class DSPTensor(torch.Tensor):
    ...
    _source: Optional["TensorSource"] = None

    @property
    def dsp_dtype(self) -> DSPDtype:
        """DSP 类型（保证 create 后非 None）。"""
        assert self._dsp_dtype is not None, "DSPTensor 未初始化 dtype"
