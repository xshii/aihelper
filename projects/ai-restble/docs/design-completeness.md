...
| ID | 待办 | 现状 |
|----|------|------|
| **P1.1** | `merge-spec.md` 还有 `@unit / @level` 标为 reserved 的注解 | 当前完全未实现；当前文档已删除 reserved 行（前轮修过） — **需求已退场？** 需 confirm |
| **P1.2** | `@index:repeatable` 的 merge 行为 | loader 解析 OK，但 `merger.py` 对 repeatable 索引的合并语义需要明确（当前是默认 conflict，可能不对） |
| **P1.3** | `validate_schema` 对 FK 字符串格式的预校验 | 当前 `_FK_RE = ^([A-Z][A-Za-z0-9_]*)\.([A-Za-z_][A-Za-z0-9_]*)$` 在 loader 静默接受不匹配（不写入 fk_targets）；应改为：写了 ref 子字段尾注释但不匹配 FK 格式 → WARNING 或 raise |

### P2 — 工程化（可做）

| ID | 待办 | 现状 |
|----|------|------|
| **E1** | CLI 入口 | `cli.py` 当前是 Flask app 启动；缺 `unpack <xml> <out_dir>` / `pack <fixture_dir> <out_xml>` / `merge <inputs...>` 三个命令 |
| **E2** | template/ 一致性提案 | `_children_order.yaml` 当前 fixture 比 preprocess 多 3 行教学注释，已豁免 bit 一致；如果想 100% 一致：要么删 fixture 注释，要么 preprocess 注入注释。**当前选择：豁免**（决策已落到 `TestUnpackFolderIdentity`） |
| **E3** | `_attach_use_comments` 列对齐 | 当前用 ruamel 默认（值后 2 空格）；fixture 历史值是 10 空格 padding；已统一为前者。如果要 fixture 风格保留，需要计算列号 |
| **E4** | `unpack` 接受字符串路径（鸭子类型） | 当前签名 `Path`，传 `str` 会被 `Path()` 包好；类型签名严格一些（用 `Union[str, Path]` 或显式 `PathLike`） |
| **E5** | preprocess.py 的 `_classify` 只识别 `ResTbl` 一种 wrapper | 如果将来出现 `<OtherWrapper X="Y"/>` 形态，需要扩展 wrapper 列表（当前是写死 `WRAPPER_TAG = "ResTbl"` 单值）。可改成 `WRAPPER_TAGS: Set[str]` |

### P3 — 文档/教学（次要）

| ID | 待办 |
|----|------|
| **T1** | `docs/yaml-schema.md` 的 R7 表格删了 sum/bitmap 行，但 R7 文字还有提到 — 检查文档自洽 |
| **T2** | `prompts/skill-preprocess.md` Example 1 写了 FileInfo.yaml 首行 `# @element:<self>`，但实际 fixture 无首行 — 文档错 |
| **T3** | 没有架构总览图（subpackage 依赖关系图）；`docs/` 缺 `README.md` 索引 |
| **T4** | `merge-spec.md` 与 `yaml-schema.md` 的关系没说清（前者是 merge 引擎专项，后者是基础 yaml schema）— 加引导 |
...
- 4/4 fixtures folder-identical（template/ 豁免）
- Multi-XML idempotent dedup + 4 conflict raise paths covered
