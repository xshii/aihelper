...
每个 element data yaml 文件首行：

```yaml
# @element:<X>
```

`FileInfo.yaml` 同样含 `# @element:FileInfo` 头（无例外）。
...
1. **`_children_order.yaml`** — 顶层 emit 顺序 meta（详见 §6 注解字典）
2. **`<Element>.yaml`** — 每个 element 的 **schema scaffold**（约束/范围/合并规则/FK）

布局取决于 fixture 是否含 RunMode scope：

```
template/                     无 scope fixture
├── _children_order.yaml
├── FileInfo.yaml             ← FileInfo schema
└── <Element>.yaml            ← 每个 element 一份
```

```
template/                     有 scope fixture
├── _children_order.yaml
├── shared/
│   ├── FileInfo.yaml
│   └── <SharedElement>.yaml  ← 跨 RunMode 的 element schema
└── 0x00000000/                ← fake runmode 占位（同 element 不同真实
    └── <ScopedElement>.yaml      RunMode 同形 schema 只存一份）
```

**`<Element>.yaml` 内容形态**：镜像 data yaml 的扁平 mapping 结构，所有字段
值为 null（占位），EOL 注释槽供使用者填 ``@merge / @range / @enum / @index``
/ FK (``Table.col``) 等约束。例（wrapper 形态）：

```yaml
# @element:ResTbl
LineNum: # @related:count(Line)
- ChannelId:        # ← 后填 @range:0-15 等
  SrcType:          # ← 后填 @enum:MEM,DEV,...
  DstType:
  BurstSize:
```

**职责分离**：
- pack/unpack 主流程**不读不写** `<Element>.yaml`，只读 `_children_order.yaml`
- 由独立的 ``ecfg.legacy.scaffold.generate_scaffolds`` 显式生成 scaffold
- 字段顺序 = mapping insertion order（XML emit 按此顺序）；data yaml 与 template
  各自独立维护字段顺序，**互不约束**
...
| 文件路径 | 内容 | 说明 |
|---|---|---|
| `template/_children_order.yaml` | 顶层 ``{FileInfo: [<children>]}`` 嵌套 mapping | XML 顶层元素的 **emit 顺序**。yaml 自身结构表达"FileInfo 是文档根，list 是其 children 顺序"。``children`` list 每条 entry **只有两种形式**：(1) **`<Element>` element-class catch-all**（无 `:`）：匹配 ``resolved element name == entry`` 的所有未消费文件，按 ``(stem, full path)`` 字母序排列。(2) **`<element>:<stem>` 特例**（有 `:`）：精确 pin 单个 instance（如 ``ResTbl:CapacityRunModeMapTbl`` 让特定 wrapper 排到指定位置）。匹配按 list 顺序贪心，每文件最多匹配一次；特例先消费，catch-all 后兜底。**协议契约**：同 element 类内 ``(stem, full path)`` 字母序 == XML idx 序——不一致时必须用特例显式 pin。**文件夹位置（shared/ vs scope/）通过 path 进入 tiebreak**。所有 fixture 必备 |
...
# template/_children_order.yaml — 复合 fixture（多 element type + 前置实例需 pin）
FileInfo:
