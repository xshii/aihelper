...
| `sum(<T>.<col>)` | 跨表累加 |  |
| `bitmap(<T>.<col>, where=cond)` | 条件位图聚合 |  |
...
Template 跟数据 yaml **结构同构**，规则差异只有三点：

1. **值是占位**——`""` (string) / `0` (int) / `0x0` (hex) / `[]` (list)。类型由字面量推断，**无需 `@type`**。
...
template/ 镜像数据目录的 scope-class 分层，但只有两个固定 scope：

```
template/
├── shared/           对应数据的 shared/ —— 跨 RunMode 共享表的 schema
│   ├── FileInfo.yaml
│   ├── RatVersion.yaml
│   ├── DmaCfgTbl.yaml
│   └── CapacityRunModeMapTbl.yaml         （wrapper 形态）
└── 0x00000000/       占位 scope —— 适用于任何 0x<RunMode>/ 数据
    ├── RunModeTbl.yaml
...
**Schema 查找规则：** 数据文件 `<scope>/<stem>.yaml` 的 schema：
- scope = `shared/` → `template/shared/<logical-stem>.yaml`
- scope = `0x<hex>/` → `template/0x00000000/<logical-stem>.yaml`
- `<logical-stem>` = 数据文件 stem 去 variant 部分
...
# @element:ResTbl
LineNum: # @related:count(Line)
- ClkName: ""           # @enum:SYS_CLK,CPU_CLK,DSP_CLK
  FreqMHz: 0            # @range:1-2000
  Source: ""            # @related:ClockSource.Name
```

对应数据 yaml（如 `0x10000000/ClkCfgTbl.yaml`，bare 形态）：
