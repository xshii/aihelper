# platforms/fpga/smoke/ — FPGA 冒烟入口脚本坑位

冒烟用例的实际触发脚本。由 `platform.yaml` 的 `smoke_entry` 引用。
...
3. **stdout 关键字**：建议输出 `SMOKE COMPLETE`（成功）或 `FATAL ...`（早期失败），被 deploy.py keyword 监听用于早终止
