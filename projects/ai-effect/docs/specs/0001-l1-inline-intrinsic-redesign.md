...
`hardware_macros`(`hac_3r`…,V0.1) · word 语义映射(V0.1) · 覆盖描述符 · `pa_dump_enabled`。
...
- **Q3** 目标上 macro 紧跟其 intrinsic 的**执行顺序是否可靠**(bracketing 前提);异步/乱序怎么兜。
- **Q4** 寄存器「PC 运行版」确切格式。
- **Q5** 硬件宏完整清单 + 各自 word 数/语义;宏 word 之外的「其他入参」是什么;inline 调用链最深几级。
- **Q6** intrinsic 头文件能否就地插桩(只读?生成副本?),与构建系统如何配合。
